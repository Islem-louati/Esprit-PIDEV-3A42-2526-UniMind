...
class Question
{
    ...
    #[Assert\Choice(
        ...
        message: "Le type de question '{{ value }}' n'est pas valide."
    )]
    ...
    #[Assert\Count(min: 2, minMessage: "Une question doit avoir au moins {{ limit }} options.")]
    #[Assert\All([
        new Assert\NotBlank(message: "Une option ne peut pas être vide."),
        new Assert\Length(max: 255, maxMessage: "Une option ne peut pas dépasser {{ limit }} caractères.")
    ])]
    private array $options_quest = [];

    #[ORM\Column(type: Types::JSON)]
    #[Assert\NotNull(message: "Les scores ne peuvent pas être nuls.")]
    #[Assert\Count(min: 2, minMessage: "Les scores doivent correspondre au nombre d'options.")]
    #[Assert\All([
        new Assert\Type(type: 'integer', message: "Le score doit être un nombre entier."),
        new Assert\Range(min: -100, max: 100, notInRangeMessage: "Le score doit être compris entre {{ min }} et {{ max }}.")
    ])]
    ...
    {
        // ✅ CORRECTION : suppression de htmlspecialchars
    ...
    {
        // ✅ CORRECTION : suppression de htmlspecialchars
        $options_quest = array_map('trim', $options_quest);
        $options_quest = array_values(array_unique($options_quest));
    ...
    {
        $option = trim($option);
        if (empty($option)) throw new \InvalidArgumentException("L'option ne peut pas être vide.");
        if (strlen($option) > 255) throw new \InvalidArgumentException("L'option ne peut pas dépasser 255 caractères.");
        if ($score < -100 || $score > 100) throw new \InvalidArgumentException("Le score doit être compris entre -100 et 100.");
        if (in_array($option, $this->options_quest, true)) throw new \InvalidArgumentException("Cette option existe déjà.");
    ...
    {
        $index = array_search(trim($option), $this->options_quest, true);
        return ($index !== false && isset($this->score_options[$index])) ? $this->score_options[$index] : null;
    }

    public function getScoreForOptionIndex(int $index): ?int
    {
        if ($index < 0) throw new \InvalidArgumentException("L'index doit être un entier positif.");
    ...
    public function validateOptionsAndScores(): bool
    {
        ...
        foreach ($this->options_quest as $i => $option) {
            if (!is_string($option) || trim($option) === '') {
                throw new \InvalidArgumentException("L'option à l'index {$i} ne peut pas être vide.");
            }
        }
        foreach ($this->score_options as $i => $score) {
            if (!is_int($score) || $score < -100 || $score > 100) {
                throw new \InvalidArgumentException("Le score à l'index {$i} est invalide.");
            }
        }
    ...
    {
        return empty($this->score_options) ? 0 : max($this->score_options);
    }

    public function getMinScore(): int
    {
        return empty($this->score_options) ? 0 : min($this->score_options);
    }

    public function calculateScoreForResponse($response): ?int
    {
        if ($response === null) return null;
        if (is_int($response)) return $this->score_options[$response] ?? null;
        if (is_string($response)) return $this->getScoreForOption($response);
    ...
    public function getResume(): string
    {
        $optionsText = '';
        for ($i = 0; $i < count($this->options_quest); $i++) {
            $optionsText .= sprintf("\n  %d. %s (score: %d)", $i + 1, $this->options_quest[$i] ?? '', $this->score_options[$i] ?? 0);
        }
        return sprintf("Question #%d\nTexte: %s\nType: %s\nOptions:%s",
            $this->question_id ?? 'Nouvelle',
            substr($this->texte, 0, 100),
    ...
    {
        if (!isset($data['texte'])) throw new \InvalidArgumentException("Le champ 'texte' est obligatoire.");
        if (!isset($data['options']) || !is_array($data['options'])) throw new \InvalidArgumentException("Les options sont obligatoires.");
        if (!isset($data['scores']) || !is_array($data['scores'])) throw new \InvalidArgumentException("Les scores sont obligatoires.");
    ...
    public function sanitize(): void
    {
        ...
        $this->options_quest = array_map('strip_tags', $this->options_quest);
    }
}