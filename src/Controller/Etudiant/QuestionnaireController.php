...
class QuestionnaireController extends AbstractController
{
    ...
    public function passer(Questionnaire $questionnaire, EntityManagerInterface $em, TraductionService $traductionService): Response
    {
        ...
        // Traduire les questions et options en arabe
        $questionsTraduites = [];
        $optionsBase = ["Jamais", "Rarement", "Parfois", "Souvent", "Toujours"];
        $optionsTraduites = array_map(fn($opt) => $traductionService->traduire($opt, 'fr', 'ar'), $optionsBase);

        foreach ($questions as $question) {
            $texte = html_entity_decode($question->getTexte(), ENT_QUOTES, 'UTF-8');
            $questionsTraduites[$question->getQuestionId()] = $traductionService->traduire($texte, 'fr', 'ar');
        }

        return $this->render('etudiant/questionnaire/passer.html.twig', [
            'questionnaire' => $questionnaire,
            'questions' => $questions,
            'questions_traduites' => $questionsTraduites,
            'options_traduites' => $optionsTraduites,
        ]);
    }
    ...
    public function submitQuestionnaire(Request $request, Questionnaire $questionnaire, EntityManagerInterface $em): Response
    {
        ...
        // Calculer le score directement
        $scoreTotale = 0;
        foreach ($reponsesData as $questionId => $valeur) {
            $scoreTotale += (int) $valeur;
        }
        ...
        // Forcer le score calculé
        $reponse->setScoreTotale($scoreTotale);
