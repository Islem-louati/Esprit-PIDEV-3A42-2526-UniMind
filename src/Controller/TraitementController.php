...
class TraitementController extends AbstractController
{
    ...
    public function index(TraitementRepository $traitementRepository, Request $request): Response
    {
        $user = $this->security->getUser();
        ...
        if (!$user) {
            ...
            switch ($sort) {
                ...
                case 'psychologue':
                    ...
                    break;
                case 'statut':
                    $qb->orderBy('t.statut', $order)
                       ->addOrderBy('t.titre', 'asc');
                    break;
                case 'priorite':
                    // Tri personnalisé pour la priorité
                    $qb->orderBy('CASE WHEN t.priorite = \'haute\' THEN 1 WHEN t.priorite = \'moyenne\' THEN 2 WHEN t.priorite = \'basse\' THEN 3 ELSE 4 END', $order)
                       ->addOrderBy('t.titre', 'asc');
                    break;
                case 'progression':
                    $qb->orderBy('(SELECT COUNT(s.id) FROM App\Entity\SuiviTraitement s WHERE s.traitement = t.id)', $order)
                       ->addOrderBy('t.titre', 'asc');
                    break;
                default:
